...
class MainWindow:
    ...
    def create_report(self):
        self.newWindow = Toplevel(self.window)
        self.app = ModoReport(self.newWindow, window)
        self.window.withdraw()
    ...
    def get_data(self):
        ...
        tablita=data[data['tipo']!='Traspaso'].pivot_table('eur2',index=['anio','mes'],columns='tipo',aggfunc='sum')
...
class ModoInsert:
    ...
    def getMovimiento(self, tipo):
        ...
        query = 'SELECT id , Fecha , Cuenta , Categoria , Subcategoria,Descripcion,Total,TipoMov,Notas FROM pruebas Where TipoMov="' + tipo + '" order by insertupdate desc'
    ...
    def g_editar(self,tipo):
        ...
        Button(self.edit_wind, text='Refresh',
               command=lambda: self.editar_registro(id, newFecha.get(), newCuenta.get(), newCategoria.get(),
                                                    ...
                                                    newtipo.get(), newNota.get(),tipo)).grid(row=9, columns=2, sticky=W + E)
...
class ModoReport:
    ...
    def get_pie(self,data):
        ...
        self.subploti1.pie(tablita.sort_values(by='eur', ascending=False).head(5), labels=tablita.sort_values(by='eur', ascending=False).head(5).index, autopct='%1.1f%%',
